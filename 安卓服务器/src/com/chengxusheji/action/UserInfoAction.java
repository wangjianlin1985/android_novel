...
public class UserInfoAction extends BaseAction {
    ...
    public String QueryUserInfo() {
        ...
        List<UserInfo> userInfoList = userInfoDAO.QueryUserInfoInfo(user_name, name, birthday, telephone, currentPage);
        /*计算总的页数和总的记录数*/
        userInfoDAO.CalculateTotalPageAndRecordNumber(user_name, name, birthday, telephone);
        /*获取到总的页码数目*/
    ...
    public String QueryUserInfoOutputToExcel() { 
        ...
        if(birthday == null) birthday = "";
        if(telephone == null) telephone = "";
        List<UserInfo> userInfoList = userInfoDAO.QueryUserInfoInfo(user_name,name,birthday,telephone);
        ExportExcelUtil ex = new ExportExcelUtil();
        String title = "UserInfo信息记录"; 
        String[] headers = { "用户名","姓名","性别","用户照片","出生日期","联系电话"};
        List<String[]> dataset = new ArrayList<String[]>(); 
        for(int i=0;i<userInfoList.size();i++) {
        	UserInfo userInfo = userInfoList.get(i); 
        	dataset.add(new String[]{userInfo.getUser_name(),userInfo.getName(),userInfo.getSex(),userInfo.getUserPhoto(),new SimpleDateFormat("yyyy-MM-dd").format(userInfo.getBirthday()),userInfo.getTelephone()});
        }
        /*
